...
float noise(vec3 vec) 
{
    ...
    float
        ...
        v = fade(vec.y),
        w = fade(vec.z);
    int A = texture1D(p,V.x/z).x+V.y, 
        AA = texture1D(p,A/z).x+V.z,
        AB = texture1D(p,(A+1)/z).x+V.z,
        B = texture1D(p,(V.x+1)/z).x+V.y,
        BA = texture1D(p,B/z).x+V.z,
        BB = texture1D(p,(B+1)/z).x+V.z;

    return lerp(w, lerp(v, lerp(u, grad(texture1D(p,AA/z).x,vec3(vec.x,vec.y,vec.z)),
                                grad(texture1D(p,BA/z).x,vec3(vec.x-1,vec.y,vec.z))),
                        lerp(u, grad(texture1D(p,AB/z).x,vec3(vec.x,vec.y-1,vec.z)),
                             grad(texture1D(p,BB/z).x,vec3(vec.x-1,vec.y-1,vec.z)))),
                lerp(v, lerp(u, grad(texture1D(p,(AA+1)/z).x,vec3(vec.x,vec.y,vec.z-1)),
                             grad(texture1D(p,(BA+1)/z).x,vec3(vec.x-1,vec.y,vec.z-1))),
                     lerp(u, grad(texture1D(p,(AB+1)/z).x,vec3(vec.x,vec.y-1,vec.z-1)),
                          grad(texture1D(p,(BB+1)/z).x,vec3(vec.x-1,vec.y-1,vec.z-1)))));
}
